...
3. **Networked rolls:** Edge `roll-dice` only; receipts in `match_rolls` (unique `action_id`). **Networked moves (v2):** Edge `move-auth` (seed / `submit-move` / `pass-turn`) with wallet signatures; `match_states.seq` is authority. Offline keeps local `processMove`. `npm run check:engine` must pass.
