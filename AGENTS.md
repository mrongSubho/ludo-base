...
- Themes are class-based on `<body>` (`ludo-theme` cookie, see `app/layout.tsx`): `theme-retro-futurism` (default, dark) and `theme-daybreak` (soft-UI light). Legacy `dark`/`ui` cookies auto-migrate to retro; legacy `porcelain` migrates to daybreak. CSS lives in `app/globals.css` + `app/styles/themes/default.css`.
