...
3. **Networked rolls:** Edge `roll-dice` only; receipts in `match_rolls` (unique `action_id`). **Networked moves (v2):** Edge `move-auth` (seed / `submit-move` / `pass-turn` / `submit-power`) with wallet signatures; `match_states.seq` is **display + rules** authority (P4: host `ENGINE_STATE` is a hint only). Boost + pickup + powers run in Edge (`lib/engine`). Offline keeps local `processMove`. `npm run check:engine` must pass.
