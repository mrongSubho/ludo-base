-- 2. Atomic Matchmaking RPC (Fixed Type Mismatch)
-- This version fixes a bug where v_opponent_id was declared as UUID but was receiving player_id (TEXT).
...
BEGIN
    ...
    SELECT id, player_id INTO v_opponent_ticket_id, v_opponent_player_id
    ...
    IF v_opponent_ticket_id IS NOT NULL THEN
        ...
        -- Return match details and the opponent's ID
        RETURN jsonb_build_object(
            'status', 'matched', 
            'match_id', v_match_id, 
            'ticket_id', v_ticket_id,
            'opponent_id', v_opponent_player_id
        );
    ELSE
